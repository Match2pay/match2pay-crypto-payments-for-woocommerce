...
jQuery( function ( $ ) {
	const isPayForOrderForm =
		window.location.href.indexOf( 'pay_for_order' ) !== -1;
	...
	window.match2pay_ajax_action = function (
		url,
		callback,
		_method,
		_data,
	...
	window.match2pay_getPaymentFormData = function () {
		const ajaxUrlNode = document.getElementById(
			'match2pay-payment-gateway-payment-form-request-ajax-url'
		);
		...
		const url = ajaxUrl;
		const method = 'POST';
		const data = $( selector ).closest( 'form' ).serialize();
		match2pay_ajax_action(
			url,
			match2pay_getPaymentFormDataCallback,
			method,
			data,
			false
		);
	...
	window.match2pay_getWatcherData = function () {
		const ajaxUrlNode = document.getElementById(
			'match2pay-payment-gateway-watcher'
		);
		const ajaxUrl = ajaxUrlNode
			? ajaxUrlNode.getAttribute( 'data-value' )
			: null;
	...
	function showError( errorMessage, selector ) {
		let $container = $( '.woocommerce-notices-wrapper, form.checkout' );
	...
	window.match2pay_validateCheckout = function ( elem ) {
		...
		if ( elem.classList.contains( 'match2pay-order-pay' ) ) {
			const callback = match2pay_getPaymentFormDataCallback;
			const url =
				match2pay_params.ajax_url +
				'?action=match2pay_orderpay_payment_request';
		...
		const checkoutCheckUrlNode = document.getElementById(
			'match2pay-payment-gateway-start-checkout-check-url'
		);

		if ( checkoutCheckUrlNode ) {
			const url = checkoutCheckUrlNode.getAttribute( 'data-value' );
			if ( url ) {
				const callback = match2pay_validateCheckoutCallback;
	...
	function match2pay_validateCheckoutCallback( response ) {
		if (
			...
			const messageItems = response.data.messages.error
	...
	function match2pay_freeze_checkout_form() {
		const billingFieldsDiv = document.querySelector(
	...
	window.match2pay_getPaymentFormDataCallback = function ( response ) {
		...
		if ( $( '[name=match2pay_currency][type=hidden]' ).length === 0 ) {
			$( '#match2pay_embedded_payment_form_btn' ).text(
				'Change Currency'
			);
			$( '#match2pay_embedded_payment_form_btn' ).show();
		}
	...
	window.match2pay_restorePayment = function () {
		const previousPayment = $( '#match2pay-payment-form' ).data(
			'payment-id'
		);
		match2pay_createHiddenInputData(
			'match2pay_paymentId',
	...
	window.match2pay_createHiddenInputData = function ( inputId, inputValue ) {
		...
		if ( !! document.getElementById( inputId ) ) {
			...
			const checkoutForm = document.getElementsByClassName(
			...
			const orderReviewForm = document.getElementById( 'order_review' );
			if ( orderReviewForm ) {
	...
	window.match2pay_watcherCallback = function ( response ) {
		...
		const paymentStatusElement =
			document.getElementById( 'match2pay-details' );
	...
	function paymentMethodAction() {
		if (
			$( '#match2pay_currency[type=select]' ) &&
